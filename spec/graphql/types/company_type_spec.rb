...
module Types
  RSpec.describe CompanyType do
    ...
    it 'has a :autoWatermark field of Boolean type' do
      # Ensure the field is of Boolean type
      expect(subject.fields["autoWatermark"].type.to_type_signature).to eq("Boolean")
    end

    it 'has a :previewOnly field of Boolean type' do
      # Ensure the field is of Boolean type
      expect(subject.fields["previewOnly"].type.to_type_signature).to eq("Boolean")
    end

    it 'has a :subscription field of Subscription type' do
      # Ensure the field is of subscription type
      expect(subject.fields["subscription"].type.to_type_signature).to eq("Subscription")
    end
    ...
    it 'has :parents field of Company type' do
      ...
      expect(subject.fields["parents"].type.to_type_signature).to eq("[Company!]")
    end

    it 'has :brokers field of Company type' do
      # Ensure the field is of Company type
      expect(subject.fields["brokers"].type.to_type_signature).to eq("[Company!]")
    end
