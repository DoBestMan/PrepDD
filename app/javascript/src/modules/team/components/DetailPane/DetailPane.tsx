...
import Dropdown from '../../../common/OutlinedDropdown';
...
import {
  UserDetails_user,
  UserDetails_user_teams,
} from '../../../../graphql/queries/__generated__/UserDetails';
...
export default function DetailPane(props: DetailPaneProps) {
  ...
  useEffect(() => {
    ...
    if (currentUser.roles) {
      const findRole = currentUser.roles.filter(
        role => role.companyId === company
      )[0];
  ...
  useEffect(() => {
    const updatedUser = idx(
      updateTeamMemberRes,
      updateTeamMemberRes => updateTeamMemberRes.updateTeamMember.user
    );
  ...
  useEffect(() => {
    const errors = idx(
      updateTeamMemberRes,
      updateTeamMemberRes => updateTeamMemberRes.updateTeamMember.errors
    );
  ...
  useEffect(() => {
    const removedUser = idx(
      removeTeamMemberRes,
      removeTeamMemberRes => removeTeamMemberRes.removeTeamMember.user
    );
  ...
  useEffect(() => {
    const errors = idx(
      removeTeamMemberRes,
      removeTeamMemberRes => removeTeamMemberRes.removeTeamMember.errors
    );
  ...
  useEffect(() => {
    const removedUser = idx(
      ...
      removeCompanyMemberRes => removeCompanyMemberRes.removeCompanyMember.user
    );
  ...
  const handleUpdateMemberList = (updatedUser: UserDetails_user) => {
    ...
    updateMemberList({
      ...
      companies: updatedUser.companies as
        | CompanyUsers_companyUsers_users_companies[]
        | null,
      teams: updatedUser.teams,
      roles: updatedUser.roles as
        | CompanyUsers_companyUsers_users_roles[]
        | null,
    });
  };
